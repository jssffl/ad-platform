  ...
  influencers.forEach((influencer) => {
    cardList.innerHTML += `
      ...
      influencer.id === activeCard + 1 ? 'active' : ''
        ...
        <span>${influencer.youtube}</span></div>
        <div class="flex-row">
        <span class="fb-icon"><i class="fab fa-facebook-square  " ></i>
        </span>
        <span>${influencer.fb}</span></div>
        <div class="flex-row">
        <span class="ig-icon"><i class="fab fa-instagram">
       </i> </span>
       <span>${influencer.ig}</span></div>
        </div>
     `
  })
})()
